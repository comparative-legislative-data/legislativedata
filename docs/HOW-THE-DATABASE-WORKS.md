...
## 1. Think of it as one workbook with 23 tabs

Postico lists 23 things side by side and they look equally important. They are
not. There are four kinds, and only one kind is data you look after.

| Kind | How many | Spreadsheet equivalent |
|---|---|---|
| Names beginning `ref_` | 9 | Dropdown lists |
...
**The nine `ref_` tabs are dropdown lists.** They are not data about bills.
...
**One tab has data in it today: `bill_candidate`, 73 rows, Session 1.**

**Three tabs are the finished answer and are all still empty:** `bill`,
`stage_event`, `field_source`. They fill up when promotion runs.
...
One row per line of a factsheet. Thirty-six columns, which sounds like a lot
...
form: the asp number lifted out of the title into its own column, "6 October
1999" turned into a real date. Every column in this block has a counterpart in
...
There are nine because there are nine columns that have a fixed set of answers:
...
styled at the time, and which stages belong to which kind of bill.
...
Session 1 is empty. Nothing can be admitted while it is non-empty.
...
the factsheet does not print the date they were rejected at Stage 1 — that came
from the Official Report. So the bill's row says "factsheet", and one small
note beside it says: this one date, from the Official Report, seen on this
date, printed there as this.

Two things about this tab. It records the value **as the source worded it**,
not as we tidied it, so that if the Parliament later revises a record you can
see a change of wording and not just a change of value. And it is
**append-only**: rows can be added and can never be edited or deleted. Reading
a source a second time adds a second note beside the first. That is enforced,
not a convention — an attempt to change one is refused.
...
## 4. What promotion will do

Promotion is the one moving part, and it hasn't run yet. When it does, for each
row on your sheet marked `accepted`:
...
- a provenance note is filed for each cell that didn't come from the row's
  stated source — six of them, for Session 1.
...
from your staging lines, so rebuilding gives every bill its old number back and
the provenance notes — which can never be edited — keep pointing at the right
bill. And a bill cannot be put on the clean sheet at all unless there is a
staging line behind it, so nothing can get in except through the gate.
