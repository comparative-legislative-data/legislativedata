...
## Contents — 204 decisions over 10 days
...
- [The chart colours are the ones that passed the palette check, and a figure opens its bills on the Data page](#2026-09-19--the-chart-colours-are-the-ones-that-passed-the-palette-check-and-a-figure-opens-its-bills-on-the-data-page)
- [Thought 2's figures are carried in the copy, and go live only with the chart](#2026-09-19--thought-2s-figures-are-carried-in-the-copy-and-go-live-only-with-the-chart)
- [Thought 2's write-up is agreed: what the outcomes chart counts, shows and says, and how its figures are carried](#2026-09-19--thought-2s-write-up-is-agreed-what-the-outcomes-chart-counts-shows-and-says-and-how-its-figures-are-carried)
- [The Insights page's shape, frame and route to the bills, confirmed on the whole-page mock-up](#2026-09-19--the-insights-pages-shape-frame-and-route-to-the-bills-confirmed-on-the-whole-page-mock-up)
- [Strand 3 is planned as a whole before any chart is built, and no mock-up's function is lost without the owner's OK](#2026-09-19--strand-3-is-planned-as-a-whole-before-any-chart-is-built-and-no-mock-ups-function-is-lost-without-the-owners-ok)
...
## 2026-09-19 — The chart colours are the ones that passed the palette check, and a figure opens its bills on the Data page

**The owner's decisions**, on `docs/STRAND-3-THOUGHT-2-PAGE-BUILD.md`,
"agreed" to the palette check and points A to F, before building.
- **The palette check**, run once for all six charts: the outcomes'
  full breakdown failed for red-green colour-blindness in both modes, and
  in light mode Passed, In progress and the first quarter were too pale
  against the page. **The adjusted colours** in the build note become the
  site's chart colours, seen by the owner beside the approved ones and as
  a colour-blind reader sees them
  (https://claude.ai/artifact/C92BifSandiEYXCyAL58pq). The sessions'
  colours passed unchanged. Two of the checker's tests, a middle band of
  lightness and no greys, are not applied: the greys are deliberate.
- **A.** A figure opens the Data page's table showing exactly its bills,
  named above it ("The 62 bills behind: Session 5 · Government Bills ·
  Passed"), with Show all bills. "Bills" opens its session and type's
  outcome lists together; "of which became Acts" the Passed bills that are
  Enacted.
- **B.** The CSV is the copy's file, all its lines, with `sources`,
  `date_copy_taken` and `chart_address` added.
- **C.** ECharts 6.1.0, served from the machine with its licence.
- **D.** A tab and the headline figures appear only once built.
- **E.** The page's own words, in full, as in the build note.
- **F.** The site is deployed first and shows the chart only when the copy
  holds its figures; the refresh then brings it live. Each step has its
  own undo.

**Why:** the check was agreed on 17 September to be run once and recorded
so later charts do not reopen it; the smallest change that passes keeps the
look the owner approved. Opening a figure on the Data page uses the table
every reader already has, with each bill opening as it does now.

**What would reopen it:** a new colour, which is checked first; or a figure
whose bills cannot be named by the line it is in the copy.

---

## 2026-09-19 — Thought 2's figures are carried in the copy, and go live only with the chart

**Agreed by the owner** on `docs/STRAND-3-THOUGHT-2-FIGURES-BUILD.md`,
points A to E, before building.
- **A. When:** built and rehearsed now; no live copy is taken with the
  figures until the page is deployed, so the Data page never describes a
  chart that is not there.
- **B.** The Data page's What each heading holds lists
  `outcomes_by_session_and_type` last, after `terms`.
- **C.** The zip's readme says, after the working files: "The figures
  behind each chart on the Insights page are not in this download. Each is
  worked out from the files here by its working, and can be downloaded
  beneath its chart; about.csv counts their lines."
- **D.** Every note's `applies_to` names the new file's headings it bears
  on, as for the days between stages.
- **E.** The file is compared at each refresh like every other, so a
  bill's changed outcome shows as the figures that moved; the first copy
  with it counts 720 lines added.

**Found in rehearsal, and settled in the build:** C's sentence is said only
by a copy that has a chart's figures. Said always, it stopped the undo: the
copy before could no longer remake its own zip byte for byte. It is also
only then true.

**Why:** the figures follow what strand 1 settled for the days between
stages, and go live in one step with the chart they belong to.

---

## 2026-09-19 — Thought 2's write-up is agreed: what the outcomes chart counts, shows and says, and how its figures are carried

**The owner's decisions** on `docs/STRAND-3-THOUGHT-2.md`, agreed in full.
- **Its three open points**: where Passed and became an Act differ, the
  hover says so and the table of numbers has an "of which became Acts"
  column (M5); the all-sessions panel is kept on its own scale, headed
  "All sessions together"; the Forth Crossing Bill's dropdown is kept.
- **Its working**, in the published copy's own column names, works out
  every figure for every choice, 720 lines, each with the bills it counts.
  Run on the live copy, read only, it matched the mock-up in every cell.
- **How it is carried**: a worked-out file in the copy,
  `outcomes_by_session_and_type`, made by the refresh with its working kept
  in `workings`; the copy's check refuses figures that do not add up, a
  figure whose bills do not match its count, or a bill in two outcomes of
  one arrangement; the CSV beneath the chart is that file, with the
  sources, date and address added; the working joins the zip and the
  figures do not; clicking a figure opens its bills from the file's list.
- **The sources line inside the chart's frame** is taken from the copy's
  `terms`, naming each source the chart's facts come from: Scottish
  Parliament; legislation.gov.uk; legislativedata.org. Checked against the
  sources file: not the Supreme Court.
- **An explanation added**: the Forth Crossing Bill is counted as a
  government bill unless the reader chooses otherwise (M4).
- **The public wording**, and the file's descriptions, as written there.

**Why:** a chart is built to its write-up, and its closure test checks
the chart against it.

---

## 2026-09-19 — The Insights page's shape, frame and route to the bills, confirmed on the whole-page mock-up

**The owner's decisions**, on the whole-page mock-up
(https://claude.ai/artifact/XMz3FuH4zQnmNSwfM9WQVq): "works, and both look
REALLY good."
- **The page's shape**: the headline figures first, always in view, then
  three tabs: Outcomes, Time taken, When bills are introduced. "Happy."
- **One frame for every chart**, in the order `docs/STRAND-3-PLAN.md`, B,
  sets out. Agreed, **but the tabs "need to be MUCH clearer"**: on Insights
  they are three large boxes under a heading, each with its name and a line
  saying what it holds, the open one shaded with a bar of the accent across
  its top. **The Data page's tabs stay as they are.**
- **Every figure opens to the bills it counted**, in the table of every
  bill showing exactly those bills, with the figure named above. Agreed.
- **The description of a chart is shown beneath it**, headed
  "Description", as well as read out to a reader who cannot see it: "can
  stay". Being part of the frame, every chart has one.
- **The headline figures' explanations are folded** under "How these are
  counted", so the tabs stay near the top. Agreed.

**Why:** seen on the page, rather than described. The mock-up reuses the 17
September mock-ups' figures and drawing unchanged, so nothing they could do
was lost; the count behind every figure was checked to match it.

**What would reopen it:** a chart whose frame cannot hold what its write-up
needs, or the tabs still being missed once real readers use the page.

---

## 2026-09-19 — Strand 3 is planned as a whole before any chart is built, and no mock-up's function is lost without the owner's OK

**The owner's decision**, on reading `docs/STRAND-3-PLAN.md`: "I am
content to approve the plan." Asked for because the session was starting
thought 2 without the page and the shared parts planned.

**What it settles.** The plan's order: a whole-page mock-up of Insights in
the site's own look, then thought 2's write-up, then thought 2 built with
everything the six share, then each chart in the agreed order. Its three
proposals are approved and confirmed on that mock-up: the Insights page in
the Data page's shape (the headline figures, then tabs for outcomes, time
taken, and when bills are introduced); one frame for every chart; and
every figure reaching the bills it counted, through the table of every bill
showing exactly those bills.

**The owner's rule, given with it:** nothing a mock-up can do is lost in
the build unless that is essential, and then only with the owner's OK. The
plan lists what each mock-up does, read from its code; that list is the
checklist, and each chart's closure test checks it.

**Why:** the six charts share a page, a frame, dropdowns and a route to
the bills. The first chart builds all of those, so they have to suit all
six before it is built.
