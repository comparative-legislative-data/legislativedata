-- db/049_a_bill_that_fell_at_dissolution_is_checked_against_the_date.sql
--
-- Settled by the owner on 2026-09-12, alongside db/047 and db/048.
--
-- The closure test written for Sessions 1 and 2 states, among the things it
-- cannot check: "That 'fell at dissolution' can be re-derived. Seven bills are
-- coded that way because their concluding date matched the dissolution date,
-- but no dissolution date is recorded anywhere in the database -- the session
-- tab is empty. The coding cannot be checked from the data as it stands."
--
-- db/048 put the dates in. This makes the checker use them, and that stated
-- limit goes away.
--
-- WHAT IT REQUIRES. A staging line whose outcome is fell_dissolution must have
-- concluded on its session's last day. If it concluded on another day, or on no
-- recorded day at all, the line is a problem and the session cannot be promoted
-- until it is settled.
--
-- WHAT IT DELIBERATELY DOES NOT REQUIRE. The converse: a fallen bill that
-- concluded on the session's last day is not required to be coded
-- fell_dissolution. A bill can be rejected at Stage 1 on the final sitting day,
-- and a rule that refused that would be a block nobody could clear. That
-- direction is already covered where it belongs -- at entry, where
-- tools/extract_factsheet.py proposes fell_dissolution when the dates match and
-- flags every other fallen bill for review. A proposal a person reviews is the
-- right instrument for an inference; a refusal is not.
--
-- NOTHING IS INFERRED FROM SILENCE. A bill with no outcome recorded is not
-- treated as having fallen. The factsheets say which bills fell; only why is
-- ours to work out. An empty outcome means not yet coded, and three kinds of
-- bill would be miscoded by any rule that read it otherwise: one that passed
-- and awaits Royal Assent, one whose assent was blocked -- which by the
-- decision of 2026-09-10 does not fall at dissolution -- and one simply not
-- reached yet.
--
-- No bill's coding changes here. All seven that carry fell_dissolution already
-- concluded on their session's last day; db/048 asserts it.

\set ON_ERROR_STOP on
BEGIN;

-- ---------------------------------------------------------------------------
-- 1. The checker gains the rule
-- ---------------------------------------------------------------------------
...
line_checks AS (
    ...
    CROSS JOIN LATERAL (VALUES
        ...
        (CASE WHEN c.outcome = 'fell_dissolution' AND s.date_session_end IS NOT NULL
...
COMMENT ON VIEW v_candidate_problems IS
 'One row per problem with a staging line or a stage-dates row. Empty before promotion, or the promotion will refuse the session. Extended at db/015; amended at db/020, where the date_concluded rule became a rule about enactment rather than about passing; at db/028 it also flags an SP Bill number or a stated introduced title left inside short_title; at db/031 it requires a route for every Stage 1 rejection, the announcement that route rests on, and the date the Official Report was read. At db/033 it reads the stage-dates staging sheet: the date checks moved there, and it checks that stage dates run in order, stage names fit the bill type, nothing comes after the stage a bill ended at, two sources for the same stage agree, and a stage completed on a date not known has a note. At db/039 it checks that a stage marked as one that did not happen belongs to a Private Bill, has no date, is not also completed or where the bill ended, and carries a note. A problem with a stage-dates row names the stage and its source, and gives the row in stage_candidate_id. At db/042 it requires a date that differs from the factsheet''s own printed words to carry a "Checked: ... = date (source, address, date read)" citation in review_note, so the factsheet is never overridden silently. At db/044 it requires every accepted line to have had its dates compared against the other sources, and requires a difference the comparison recorded to have been adjudicated before the session can be promoted. At db/046 it requires a bill whose Stage 1 rejection route is recorded as some other route to carry a note saying what happened. At db/049 it requires a bill coded as having fallen at dissolution to have concluded on its session''s last day, which db/048 made knowable; the session-window checks, dormant until then, are awake from the same point. A missing date is not a problem; it is listed in v_stage_date_gaps.';

ALTER VIEW v_candidate_problems OWNER TO legdata;

-- ---------------------------------------------------------------------------
-- 3. Proof that the rule bites, planted and undone inside this transaction
--
-- A rule that installs cleanly and catches nothing is worse than none, because
-- it reads afterwards as a check that passed. One line's concluding date is
-- moved by a day, the checker is required to notice, and the change is rolled
-- back to the savepoint so nothing is left behind -- not even a changed
-- updated_at, which a compensating update would have left.
-- ---------------------------------------------------------------------------

SAVEPOINT proving_the_rule;

UPDATE bill_candidate SET date_concluded = date_concluded + 1
 WHERE candidate_id = (SELECT min(candidate_id) FROM bill_candidate
                        WHERE outcome = 'fell_dissolution');

DO $$
DECLARE n integer;
BEGIN
  SELECT count(*) INTO n FROM v_candidate_problems
   WHERE problem LIKE '%fallen at dissolution%';
  IF n <> 1 THEN
    RAISE EXCEPTION 'Refusing: a planted wrong date produced % complaint(s), expected 1. The rule does not work.', n;
  END IF;
  RAISE NOTICE 'The rule bites: a concluding date moved by one day is caught.';
END $$;

ROLLBACK TO SAVEPOINT proving_the_rule;
RELEASE SAVEPOINT proving_the_rule;
...
BEGIN
  ...
  SELECT count(*) INTO n FROM bill_candidate WHERE outcome = 'fell_dissolution';
  IF n <> 7 THEN
    RAISE EXCEPTION 'Refusing: % staging lines coded as fallen at dissolution, expected 7.', n;
  END IF;

  SELECT count(*) INTO n
    FROM bill_candidate c JOIN session s USING (session_number)
   WHERE c.outcome = 'fell_dissolution'
     AND c.date_concluded IS DISTINCT FROM s.date_session_end;
  IF n > 0 THEN
    RAISE EXCEPTION 'Refusing: % line(s) still fail the new rule after the savepoint was released.', n;
  END IF;

  RAISE NOTICE 'The checker requires a bill that fell at dissolution to have concluded on its session''s last day. Checker empty; seven lines carry that coding, all of them correct.';
END $$;
