-- db/088_the_day_a_bill_reached_a_stage.sql
--
-- A stage record gains the day the bill reached the stage, beside the day the
-- stage ended.
--
...
--
-- WHY IT IS TAKEN. Two bills went back to the Parliament after the Supreme
-- Court stopped them, and the Session 6 fact sheet gives each of them two days:
--
--     Reconsideration stage agreed on 4 February 2026.
--     The Bill was approved and ended Reconsideration Stage on 3 March 2026.
--
-- The second has a home: date_completed, the day of the decision that ended the
-- stage (methodology note M2). The first does not. It is the day the Parliament
-- agreed to take the bill back, which is the day the bill reached the stage --
-- a real, dated decision of the Parliament, printed by the source, and until
-- now there was nowhere to put it but prose. The owner chose the cell over the
-- prose: "how long did reconsideration take" is a question about two dates, and
-- a date kept as words cannot answer it.
--
-- WHAT IT RECORDS, AND WHAT EMPTY MEANS. date_reached holds the day the bill
-- reached the stage, where a source says so. EMPTY MEANS NO SOURCE HAS TOLD US.
-- It never means the same day as the stage ended, and it is not a start date
-- worked out from anything. After this change two stage records out of 1073
-- will carry it, both Reconsideration Stage records, and both arriving with
-- Session 6. Every one of the 1071 records already on the clean sheet is empty
-- and stays empty: no bill on the clean sheet has a Reconsideration Stage
-- record at all, so there is nothing to recode and nothing to recheck beyond
-- proving that.
--
-- NOT A GENERAL START DATE. Stages 1, 2 and 3 have a day the stage ended and
-- nothing else, because that is what the sources state. No fact sheet says when
-- a bill reached Stage 2, and this column is not an invitation to work one out
-- from the stage before. It is filled where a source states it and nowhere
-- else, which is the same rule bill.procedure carries at db/087 and M10.
--
-- WHERE THE RULES SIT. The clean sheet refuses a day reached after the day the
-- stage ended, and one on a stage the bill never had, with CHECK constraints:
-- the clean sheet may not hold a contradiction. The staging sheet holds what a
-- source appeared to say and the error checker complains, which is where a
-- misreading is meant to be caught. The gaps list asks for a missing day
-- reached on a Reconsideration Stage record and on no other stage, because
-- that is the only stage any source states one for.
--
-- ALSO IN THIS MIGRATION, both agreed at the same time and both small:
--
--   * 'pending' is widened to cover a bill still before the Parliament. Its
--     wording said "passed, and awaiting Royal Assent", and Session 7's bill
--     in progress has not passed. No new value, and nothing already recorded
--     changes meaning: every bill now carrying 'pending' has passed.
--
--   * Methodology note M11, beside M2.
--
-- The reconciliation display in tools/load_session.sql, which counts the old
-- type letters and therefore counts every bill in a prose fact sheet as
-- unrecognised, is changed in the same commit. It is a display, not data, so
-- it is not a migration.

BEGIN;

-- ---------------------------------------------------------------------------
-- The cell, on both sheets
-- ---------------------------------------------------------------------------

ALTER TABLE stage_event     ADD COLUMN date_reached date;
ALTER TABLE stage_candidate ADD COLUMN date_reached date;

COMMENT ON COLUMN stage_event.date_reached IS
 'The day the bill reached this stage, where a source states it. Empty means no source has told us, which is every stage record but the two Reconsideration Stage records: no fact sheet states when a bill reached Stage 1, 2 or 3, and this is never worked out from the stage before. The day the stage ended is date_completed. See methodology note M11.';

COMMENT ON COLUMN stage_candidate.date_reached IS
 'The day the bill reached this stage as the source appeared to say it, waiting for review. Empty means the source did not state one. It becomes stage_event.date_reached when the session is promoted. See methodology note M11.';

-- The clean sheet may not hold a contradiction. A day reached after the day the
-- stage ended is one; so is a day reached on a stage the bill never had, which
-- the did_not_happen rule already says must be empty of everything else.
ALTER TABLE stage_event
  ADD CONSTRAINT stage_event_reached_before_it_ended
  CHECK (date_reached IS NULL OR date_completed IS NULL
         OR date_reached <= date_completed);

ALTER TABLE stage_event
  ADD CONSTRAINT stage_event_a_stage_that_did_not_happen_was_not_reached
  CHECK (NOT did_not_happen OR date_reached IS NULL);
...
-- ---------------------------------------------------------------------------
--
-- Written out in full from db/087, its last text, with three checks added to
-- stage_checks and nothing else touched. The rebuilt view is diffed as TEXT
-- against the running one before this is applied, never compared by counting
-- lines: db/062 and db/086 both had to say that a view rebuilt from an older
-- migration quietly carries that migration's stale names with it, and a line
-- count cannot tell a reformat from a lost check.
...
-- ---------------------------------------------------------------------------
--
-- Written out in full from db/087, its last text, with one branch added and
-- nothing else touched. Its stage-rows CTE already carries every column of the
-- stage-dates sheet through t.*, so the new cell arrives in it unaided.
-- Diffed against the running view before this is applied, for the same reason
-- the error checker is.
...
INSERT INTO methodology_note (code, title, body, applies_to, sort_order) VALUES
 ('M11',
  'A stage record holds the day a stage ended, and sometimes the day it was reached',
  'M2 says that a stage is completed on the date of the decision that ended it, and that is the date almost every stage record in this resource holds. It is the only date the fact sheets state for Stages 1, 2 and 3.'
    || E'\n\n' ||
    'One stage is different. A bill that has passed and been stopped before Royal Assent can be taken back by the Parliament for a Reconsideration Stage, and the Session 6 fact sheet prints two days for it: the day the Parliament agreed to reconsider the bill, and the day it approved the bill and the stage ended. Both are decisions of the Parliament and both are dated, so both are recorded — the first in date_reached, the second in date_completed.'
    || E'\n\n' ||
    'An empty date_reached means no source has told us when the bill reached that stage. It does not mean the stage was reached and ended on the same day, and it is never worked out from the stage before: a stage record says what a source stated and nothing else. At the time of writing two stage records carry it, both Reconsideration Stage records, and both are bills of Session 5 reconsidered in Session 6 — the UNCRC (Incorporation) Bill and the European Charter of Local Self-Government (Incorporation) Bill.'
    || E'\n\n' ||
    'So a duration built from stage records is a duration between days the stages ended, for every stage but this one. Anything that measures how long a Reconsideration Stage took should say which two dates it used, because only that stage has the choice.',
  '{stage_event.date_reached,stage_event.date_completed}',
  11);


-- ---------------------------------------------------------------------------
-- A bill still before the Parliament is awaiting an Act too
-- ---------------------------------------------------------------------------
--
-- The wording said 'Passed, and awaiting Royal Assent'. Session 7's bill in
-- progress has not passed, and 'not_enacted' would say it had finished and
-- failed. No new value: the state is "no Act yet, and nothing adverse", which
-- covers both. Nothing already recorded changes meaning, because every bill
-- carrying 'pending' today has passed.

UPDATE ref_enactment_status
   SET definition = 'No Act yet, and nothing adverse recorded: a bill still '
                     'before the Parliament, or one that has passed and is '
                     'awaiting Royal Assent. A bill referred to the Supreme '
                     'Court or subject to a section 35 order is ''blocked'', '
                     'not ''pending''.'
 WHERE code = 'pending';
...
-- ---------------------------------------------------------------------------
--
-- Nothing on the clean sheet may move. One cell is added to each stage sheet
-- and neither is filled; the two lists are rebuilt and both must still be
-- empty. The checks that prove the new rules bite are in
...
BEGIN
  ...
  SELECT count(*) INTO n FROM stage_event WHERE date_reached IS NOT NULL;
  IF n > 0 THEN RAISE EXCEPTION 'Refusing: % stage record(s) have a day reached. None should, yet.', n; END IF;

  SELECT count(*) INTO n FROM stage_event WHERE stage = 'reconsideration';
  IF n > 0 THEN RAISE EXCEPTION 'Refusing: % Reconsideration Stage record(s) on the clean sheet. There should be none until Session 6 is promoted.', n; END IF;
  ...
  SELECT count(*) INTO n FROM methodology_note WHERE code = 'M11';
  IF n <> 1 THEN RAISE EXCEPTION 'Refusing: M11 was not written.'; END IF;

  RAISE NOTICE 'One cell on each stage sheet, both empty; error checker and gaps list both empty; M11 written.';
END $$;
