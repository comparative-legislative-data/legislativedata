-- db/058_the_official_report_is_cited_on_the_line.sql
--
-- Found by rehearsing Session 3's promotion, not afterwards.
--
-- db/055 read five Session 3 bills' endings out of the Official Report and
-- quoted the Presiding Officer on each line. It recorded the address of the
-- report it read on the stage-dates row, and nowhere else. Promotion writes a
-- bill's provenance note from the line, and takes the reference out of the
-- line's own note, so it had nothing to cite.
--
-- Three of the five are Stage 1 rejections, and promotion has refused a route
-- with no citation since db/032; the rehearsal stopped there and wrote nothing.
-- The other two -- the Budget (Scotland) (No. 2) Bill, rejected at Stage 3, and
-- the Creative Scotland Bill, which fell for want of a financial resolution --
-- would have gone onto the clean sheet with the reference empty, and nothing
-- would have said so.
--
-- Two things, then, and both are needed before Session 3 can be promoted:
--   1. the address put on all five lines, in the place Sessions 1 and 2 put it;
--   2. the error checker asking for it, so no later session can repeat this.
--
-- No new source, and nothing retyped: the address written onto each line is
-- read out of that bill's own Official Report stage row, which db/055 wrote.
...
-- ---------------------------------------------------------------------------
-- 1. The address, onto the line
--
--    Sessions 1 and 2 put it straight after the sentence saying what happened,
--    because that is what promotion cuts the value seen at: everything before
--    the address is what the Official Report said, everything after it is our
--    own commentary. The anchor below is the end of that first sentence, and
--    each one must appear exactly once or nothing is written.
-- ---------------------------------------------------------------------------

CREATE TEMP TABLE citing (candidate_id integer, title_fragment text, anchor text)
  ON COMMIT DROP;

INSERT INTO citing VALUES
 (210, 'Autism',                'Stage 1, 12 January 2011.'),
 (211, 'Budget (Scotland)',     'that it be passed.'),
 (213, 'Creative Scotland',     'the financial resolution was not agreed.'),
 (214, 'End of Life Assistance','Stage 1, 1 December 2010.'),
 (216, 'Protection of Workers', 'Stage 1, 22 December 2010.');

-- The address each line will carry, taken from its own stage row.
CREATE TEMP TABLE citing_url ON COMMIT DROP AS
SELECT a.candidate_id, a.anchor, c.review_note, s.source_ref AS url
  FROM citing a
  JOIN bill_candidate c USING (candidate_id)
  JOIN stage_candidate s ON s.candidate_id = a.candidate_id
                        AND s.source = 'official_report'
 WHERE c.short_title ILIKE '%' || a.title_fragment || '%';

DO $$
DECLARE n integer;
BEGIN
  SELECT count(*) INTO n FROM citing_url;
  IF n <> 5 THEN
    RAISE EXCEPTION 'Refusing: expected 5 lines to cite, matched %. A line number or title is wrong, or a bill has more than one Official Report stage row.', n;
  END IF;

  SELECT count(*) INTO n FROM citing_url WHERE url IS NULL OR url !~ '^https?://';
  IF n > 0 THEN
    RAISE EXCEPTION 'Refusing: % line(s) have no address on their stage row to copy.', n;
  END IF;

  SELECT count(*) INTO n FROM citing_url WHERE review_note ~ 'https?://';
  IF n > 0 THEN
    RAISE EXCEPTION 'Refusing: % line(s) already carry an address. This migration has run before, or the note has changed.', n;
  END IF;

  -- The anchor must be there, and be there once, or the address lands in the
  -- wrong place and the value seen is cut in the wrong place with it.
  SELECT count(*) INTO n FROM citing_url
   WHERE (length(review_note) - length(replace(review_note, anchor, ''))) / length(anchor) <> 1;
  IF n > 0 THEN
    RAISE EXCEPTION 'Refusing: % line(s) do not contain their anchor exactly once.', n;
  END IF;
END $$;

UPDATE bill_candidate c
   SET review_note = replace(c.review_note, u.anchor, u.anchor || ' ' || u.url)
  FROM citing_url u
 WHERE c.candidate_id = u.candidate_id;

-- ---------------------------------------------------------------------------
-- 2. The error checker asks for it
--
--    Copied from db/055 with one rule added. Everything else is as db/055
--    left it.
-- ---------------------------------------------------------------------------
...
--
--    The last two prove the thing this migration exists for: that what
--    promotion will read off each line is the address, and the words the
--    Official Report actually printed. They apply promotion's own expressions
...
DECLARE n integer;
BEGIN
  SELECT count(*) INTO n FROM citing_url u JOIN bill_candidate c USING (candidate_id)
   WHERE c.review_note !~ 'https?://';
  IF n > 0 THEN RAISE EXCEPTION '% line(s) still carry no address.', n; END IF;

  SELECT count(*) INTO n FROM citing_url u JOIN bill_candidate c USING (candidate_id)
   WHERE substring(c.review_note from 'https?://\S+') IS DISTINCT FROM u.url;
  IF n > 0 THEN
    RAISE EXCEPTION '% line(s): the first address in the note is not the one on its stage row.', n;
  END IF;

  SELECT count(*) INTO n FROM citing_url u JOIN bill_candidate c USING (candidate_id)
   WHERE coalesce(btrim(regexp_replace(
           regexp_replace(c.review_note, '^Outcome from the Official Report, not the factsheet:\s*', ''),
           '\s*https?://.*$', '')), '') = '';
  IF n > 0 THEN
    RAISE EXCEPTION '% line(s): nothing is left as the words the Official Report printed.', n;
  END IF;

  SELECT count(*) INTO n FROM bill_candidate c
   WHERE c.stage_1_rejection_route IS NOT NULL
     AND substring(c.review_note from 'Result as recorded: "([^"]+)"') IS NULL;
  IF n > 0 THEN
    RAISE EXCEPTION '% route(s) no longer quote the announcement.', n;
  END IF;

  SELECT count(*) INTO n FROM v_candidate_problems;
  IF n > 0 THEN
    RAISE EXCEPTION 'The error checker finds % problem(s) across all sessions after this.', n;
  END IF;

  RAISE NOTICE 'Five lines now cite the Official Report page they were read on, and the checker asks for it.';
END $$;
