-- db/082_the_checks_ask_about_the_bills_own_session.sql
--
-- The error checker, rebuilt so that it can read a fact sheet row which is a
-- bill already on the clean sheet, and so that it asks about the new cells
-- db/080 and db/081 added.
--
-- Agreed with the owner on 2026-09-14, as part of the piece of work STATE.md
-- puts before Session 6 is loaded. Nothing here changes what any bill is or
-- what happened to it. Sessions 1 to 5 hold no row that any of it fires on,
-- and the test of that is that the checker and the gaps list return exactly
-- what they returned before: nothing.
--
-- -------------------------------------------------------------------------
-- 1. Four checks asked about the wrong session, and have since db/048.
--
-- The staging sheet has one session cell and it means WHICH FACT SHEET THE ROW
-- WAS READ OFF, not which session the bill belongs to. Four checks compared a
-- line's dates against that session. For every row loaded so far the two are
-- the same thing, so nothing has ever fired; for a row continuing a bill from
-- an earlier session they are not, and all four would have fired on rows that
-- are correct. STATE.md has carried this since 2026-09-12 as a false alarm
-- rather than a false pass, to be dealt with before Session 6.
--
-- Two of them now ask about the session the BILL belongs to, which for an
-- ordinary line is unchanged. The other two -- passed after the session ended,
-- concluded after the session ended -- are asked only of a line that is a bill
-- in its own right, because a continuing line exists precisely to record
-- business that ran past the end of a session. What keeps such a line honest
-- instead is the stage checks, which already refuse a date before the bill's
-- introduction or after its Royal Assent.
--
-- And the first of them gains a sentence, so that the one row this is really
-- about says what to do: a line introduced before its own fact sheet's session
-- began must name the bill it continues. That is the rule that catches a
-- carried-over row whatever its title does, which title-matching cannot -- the
-- UNCRC Bill is listed as an Act in Session 6.
--
-- -------------------------------------------------------------------------
-- 2. Sixteen checks that are new.
--
-- Three on a line that continues a bill: the introduction date must be the
-- same, since that is the tie between the two rows, and only a later session
-- can continue a bill.
--
-- Nine on the two cells that record a bill having been stopped before Royal
-- Assent: filled together or empty together, each value in its list, only a
-- bill that passed can be stopped, a bill recorded as blocked must say it is
-- still blocked, a reconsidered-and-passed bill must be an enacted Act, a
-- withdrawn one must say when -- and the cell and the Reconsideration Stage row
-- must agree in both directions.
--
-- Three on a bill that carried an earlier bill's scrutiny: a stage recorded as
-- never having happened must say which bill it happened to, the earlier bill
-- must be earlier, and no line may claim both relationships at once.
--
-- -------------------------------------------------------------------------
-- 3. The rules are proved to bite, as db/062 and db/078 proved theirs.
--
-- Not here. A session may not mark the check on a rule it added itself
-- (DECISIONS.md, 2026-09-14), and this session added them. They are written up
-- as items in docs/CLOSURE-TESTS.md for a session that did not write them.
