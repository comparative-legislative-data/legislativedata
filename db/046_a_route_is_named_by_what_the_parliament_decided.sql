-- db/046_a_route_is_named_by_what_the_parliament_decided.sql
--
-- Settled by the owner on 2026-09-12, from three options.
--
-- The Proportional Representation (Local Government Elections) (Scotland) Bill
-- is the case. On 6 February 2003 Tricia Marwick moved that the Parliament agree
-- to its general principles; Iain Smith's amendment turned that into a motion
-- that did not agree to them and carried 65/54/2; the amended motion was then
-- agreed to 65/53/3. The motion passed and the bill fell.
--
-- Two changes follow from that.
--
-- Every route is now defined by WHAT THE PARLIAMENT DECIDED about the bill's
-- general principles, not by what happened to the motion. Each definition opens
-- with the decision and then says how it was reached. A rule keyed on the fate
-- of the motion would eventually read "motion agreed to" as "bill progressed".
--
-- And there is one open route, for a path nobody has anticipated, so that an odd
-- bill can be recorded truthfully instead of blocking a whole session until
-- someone writes a new code. It is the one route that cannot stand on its code
-- alone: the Presiding Officer's announcement is already required of every
-- route, and this also requires a note saying what the effect was. A second
-- case of the same kind earns its own entry rather than staying there.
--
-- No bill changes its route. The eleven rejections in Sessions 1 and 2 keep the
-- routes they had; what changes is the words a reader is given for them.

\set ON_ERROR_STOP on
BEGIN;

-- ---------------------------------------------------------------------------
-- 1. Each route says what the Parliament decided, then how
-- ---------------------------------------------------------------------------

UPDATE ref_stage_1_rejection_route SET
  label = 'The general principles were not agreed to: motion of the member in charge, disagreed to',
  definition = 'The Parliament did not agree to the bill''s general principles, by disagreeing to the motion of the member in charge that it should agree to them. The usual way a bill is rejected at Stage 1.'
 WHERE code = 'member_motion_disagreed';

UPDATE ref_stage_1_rejection_route SET
  label = 'The general principles were not agreed to: motion of the member in charge, amended to reject, agreed to as amended',
  definition = 'The Parliament did not agree to the bill''s general principles, by agreeing to the member in charge''s own motion after an amendment had turned it into one that did not agree to them. The motion passed and the general principles fell, so the fate of the motion and the fate of the bill point opposite ways. The amendment, who moved it and the reason the resolution gives are in bill.note.'
 WHERE code = 'member_motion_amended_agreed';

UPDATE ref_stage_1_rejection_route SET
  label = 'The general principles were not agreed to: committee motion under Rule 9.14.18, agreed to',
  definition = 'The Parliament did not agree to the bill''s general principles, by agreeing to the motion of the convener of the lead committee, moved under Rule 9.14.18, that they not be agreed to. The rule lets the committee recommend this where, in its opinion, (a) the consultation or published material does not demonstrate a reasonable case for the policy objectives or that legislation is needed; (b) the Bill appears clearly outwith legislative competence and is unlikely to be brought within it by amendment at Stages 2 and 3; or (c) its drafting is too deficient to be put right by amendment. The motion does not state which; our view, where the grounds allow one, is in bill.note. Cited as numbered and worded in the current Standing Orders, taken to be unchanged since 2006.'
...
INSERT INTO ref_stage_1_rejection_route (code, label, definition, sort_order) VALUES
 ('other_route',
  'The general principles were not agreed to: some other route',
  'The Parliament did not agree to the bill''s general principles by some path other than those above. Usable only where the Presiding Officer''s announcement is quoted in the staging line''s review note and bill.note says what happened and what its effect was; the error checker requires both. It exists so that a bill whose procedure nobody anticipated can be recorded truthfully rather than stop a whole session, and it is not a place to leave things: a second case of the same kind earns its own entry here.',
  4);
...
DECLARE n integer;
BEGIN
  SELECT count(*) INTO n FROM ref_stage_1_rejection_route;
  IF n <> 4 THEN
    RAISE EXCEPTION 'Refusing: % routes, expected 4.', n;
  END IF;

  SELECT count(*) INTO n FROM ref_stage_1_rejection_route
   WHERE definition NOT LIKE 'The Parliament did not agree to the bill''s general principles%';
  IF n > 0 THEN
    RAISE EXCEPTION 'Refusing: % route(s) do not open with what the Parliament decided.', n;
  END IF;

  SELECT count(*) INTO n FROM bill WHERE stage_1_rejection_route = 'other_route';
  IF n > 0 THEN
    RAISE EXCEPTION 'Refusing: % bill(s) already use the open route, which should be empty.', n;
  END IF;
  ...
  END IF;
  RAISE NOTICE 'Four routes, each named by what the Parliament decided. The open route is empty. Checker empty.';
END $$;
